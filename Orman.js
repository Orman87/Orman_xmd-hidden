...
const { sendMenu } = require('./start/menu');
...
const isPremiumUser = (sender) => {
    ...
    const ownerNumbers = global.db.data?.settings?.owners || ['256704291969@s.whatsapp.net'];
...
if (m.message) {
    if (isCmd && !m.isGroup) {
        ...
        console.log(chalk.white(chalk.bgHex('#4a69bd').bold(`╭─── ❁𝙾𝚁𝙼𝙰𝙽 𝚇𝙼𝙳!❁ ──╼`)))
...
newsletterName: "𝙿𝙾𝚆𝙴𝚁𝙴𝙳 𝙱𝚈 𝙱𝙻𝙰𝙲𝙺 𝚃𝙴𝙲𝙷 ™"
}
}
}, {
quoted: orman
...
𖣔 ➪ https://wa.me/256704291969?text=*ᗰOᖇᗴ+IᑎᖴOᖇᗰᗩTIOᑎ+ᗩᗷOᑌT+𝙾𝚁𝙼𝙰𝙽_𝚇𝙼𝙳+ᵇᵒᵗ*

*Don't forget to follow channel*  
  
𖣔 ➪ https://whatsapp.com/channel/0029VasAQRiGk1FtXGUz5T2V`
    }, {
        quoted: terri
    })
...
`;
    ...
    await bot.sendMessage(m.chat, {
      image: { url: 'https://files.catbox.moe/3xfxhw.jpg' },
      caption: famText,
      contextInfo: {
...
if (!args[0]) return reply('*Please provide a TikTok video url!*');
    ...
    try {
      ...
      await bot.sendMessage(
        m.chat,
        {
          caption: '> *_𝙳𝙾𝚆𝙽𝙻𝙾𝙰𝙳𝙴𝙳 𝙱𝚈 𝙾𝚁𝙼𝙰𝙽~𝚇𝙼𝙳 ᵇᵒᵗ_*',
