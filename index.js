...
async function downloadSessionData() {
  ...
  try {
    ...
    if (settings.SESSION_ID.startsWith("Orman-xmd~")) {
      ...
      throw new Error("Invalid SESSION_ID format. Use 'Orman-xmd~' for base64 or 'malvin~' for MEGA.nz");
    }
  } catch (error) {
...
say(`Created By King Orman\nYOUTUBE : BlackTech-87\nTelegram : Theking256`, {
  font: 'console',
  align: 'center',
  gradient: [randomcolor, randomcolor]
})
  ...
  if (!sessionExists && !bot.authState.creds.registered) {
    const phoneNumber = await question(`< TYPE YOUR WHATSAPP NUMBER STARTING WITH COUNTRY CODE\n<✓> EXAMPLE : 2567042*****\n <♡> Number : `);
   // Request and display the pairing code
   const code = await bot.requestPairingCode(phoneNumber.trim());
   console.log(color(`[ # ] enter that code into WhatsApp, sucker😎 : ${code}`, `${randomcolor}`));
   }
}
...
bot.ev.on('group-participants.update', async (update) => {
    ...
    for (let user of participants) {
        try {
            let pp = await bot.profilePictureUrl(user, 'image');
        } catch {
            pp = 'https://files.catbox.moe/3xfxhw.jpg'; // fallback
        }
    ...
    if (update.connection === 'open') {
        ...
        await bot.sendMessage(botNumber, { 
            text: `
*_❁ 𝙾𝚁𝙼𝙰𝙽 𝚇𝙼𝙳 ᵇᵒᵗ 𝙸𝚂 𝙲𝙾𝙽𝙽𝙴𝙲𝚃𝙴𝙳 ❁_*
${readmore}            
> ┏❐━═ *𝙾𝚁𝙼𝙰𝙽 ⁱˢ ˢᵘᵐᵐᵒⁿᵉᵈ* ═╼❐
> ┃❦•❄️ *Bot:*  *𝙾𝚁𝙼𝙰𝙽-𝚇𝙼𝙳 ᵇᵒᵗ* 🫧
> ┃❦•❄️ *Time:* ${time}
 ...
 *𝙿𝙾𝚆𝙴𝚁𝙴𝙳 𝙱𝚈 𝙱𝙻𝙰𝙲𝙺 𝚃𝙴𝙲𝙷 ™*
                  *&*              
 *𝚂𝙿𝙾𝙽𝚂𝙴𝚁𝙴𝙳 𝙱𝚈 ՏᑌKO ᗪᗴᐯՏ ᑕᖇᗴᗯ ᵗᵐ*`,
            contextInfo: {
                forwardingScore: 99,
                isForwarded: true,
                forwardedNewsletterMessageInfo: {
                    newsletterJid: '120363346921879829@newsletter',
                    newsletterName: '𝐓𝐡𝐚𝐧𝐤𝐬 𝐟𝐨𝐫 𝐜𝐡𝐨𝐨𝐬𝐢𝐧𝐠 ᵒʳᵐᵃⁿ 𝐗𝐦𝐝 ᵇᵒᵗ',
                    serverMessageId: -1
                }
            }
        });
